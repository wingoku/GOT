package com.wingoku.gameofthrones.data.network.models.responses;
...
public class ApiResponse<T> {
    ...
    public ApiResponse<T> create(Throwable error){
        return new ApiErrorResponse<>(error.getMessage().length() == 0 ? "Unknown error\nCheck network connection" : error.getMessage());
    }
    ...
    public ApiResponse<T> create(Response<T> response){
        ...
        if(response.isSuccessful()){
            ...
            String errorMsg = "";
            try {
                errorMsg = response.message() + response.errorBody().string();
            } catch (IOException e) {
                e.printStackTrace();
